...
void Bullets::update(sf::Time& clockTime)
{		
	for (size_t i = 0;i < bullets.size();i++) {
		bullets[i].update(clockTime);
		if (bullets[i].getRect().left < 0 || bullets[i].getRect().left > 1920 || bullets[i].getRect().top < 0 || bullets[i].getRect().top > 1080)
...
void Bullets::checkCollision(Enemy &enemy) {
	for (size_t i = 0;i < bullets.size();i++)
